...
| `resolve_alert_event` | `mcp:failures:write` | Mark an alert event resolved |
...
| `mcp:failures:write` | Resolve failure/alert events |
...
- **Rate limits** (when `NODE_ENV=production`) — 120 HTTP requests/min on `/mcp` ingress (every JSON-RPC request, including initialize and tool calls); **60 calls/min per tool** per bearer (30/min for `get_job_logs`, `get_job_stacktraces`, `get_failure_events`, `get_queue_metrics`, `explain_job_failure`)
